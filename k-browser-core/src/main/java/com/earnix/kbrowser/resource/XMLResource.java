...
import com.earnix.kbrowser.util.Configuration;
...
public class XMLResource extends AbstractResource {
    ...
    private static com.earnix.kbrowser.dom.nodes.Document getJsoup(InputStream stream) {
        try {
    ...
    private static com.earnix.kbrowser.dom.nodes.Document getJsoup(InputSource source) {
        return getJsoup(source.getByteStream());
    }


    private static com.earnix.kbrowser.dom.nodes.Document getJsoup(Reader reader) {
        return getJsoup(new ReaderInputStream(reader, Charset.forName("UTF-8")));
    }

    // endregion

    public static com.earnix.kbrowser.dom.nodes.Document load(InputStream stream) {
        return getJsoup(stream);
    }

    public static com.earnix.kbrowser.dom.nodes.Document load(InputSource source) {
        return getJsoup(source);
    }

    public static com.earnix.kbrowser.dom.nodes.Document load(Reader reader) {
...
//        }
        ...
        private com.earnix.kbrowser.dom.nodes.Document transform(com.earnix.kbrowser.dom.nodes.Document source) {
            return source;
        }
