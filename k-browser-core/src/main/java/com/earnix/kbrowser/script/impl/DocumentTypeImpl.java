package com.earnix.kbrowser.script.impl;

import com.earnix.kbrowser.dom.nodes.DocumentType;
import com.earnix.kbrowser.swing.BasicPanel;

/**
 * @author Taras Maslov
 * 7/13/2018
 */
public class DocumentTypeImpl extends NodeImpl implements com.earnix.kbrowser.script.whatwg_dom.DocumentType {

    private DocumentType target;

    public DocumentTypeImpl(DocumentType target, BasicPanel panel) {
        super(target, panel);
        this.target = target;
    }

    @Override
    public String name() {
        return target.attr("name");
    }

    @Override
    public String publicId() {
        return target.attr("publicId");
    }

    @Override
    public String systemId() {
        return target.attr("systemId");
    }
}
